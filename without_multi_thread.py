...
# Função para encontrar dados faltantes entre duas páginas
...
if __name__ == "__main__":
    ...
    if not pd.io.common.file_exists(csv_file_path):
        with open(csv_file_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            # Colunas que vao ser geradas no seu csv
            writer.writerow(["query_string", "sku_id", "categories"])

    # Arquivo que irá ler no pandas
    df = pd.read_excel("lastcart - C&V.xlsx")

    for index, row in df.iterrows():
        process_sku(row, client, domain, csv_file_path)
